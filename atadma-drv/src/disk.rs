...
// It seems WDK crate neither defined this structure nor included related definitions.
// https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddscsi/ns-ntddscsi-_ata_pass_through_direct
#[repr(C)]
...
extern "C" 
{
	pub fn ObReferenceObjectByName(ObjectPath:PUNICODE_STRING,Attributes:ULONG,PassedAccessState:PACCESS_STATE,DesiredAccess:ACCESS_MASK,ObjectType:POBJECT_TYPE,AccessMode:KPROCESSOR_MODE,ParseContext:PVOID,ObjectPointer:*mut PVOID)->NTSTATUS;
...
fn issue_ata_cmd(device:*mut _DEVICE_OBJECT,flags:u16,cmd:u8,buffer:*mut c_void)->NTSTATUS
{
	...
	{
		IoBuildDeviceIoControlRequest(IOCTL_ATA_PASS_THROUGH_DIRECT, device, &mut req as *mut AtaPassThroughDirect as *mut c_void, size_of::<AtaPassThroughDirect>() as u32, &mut req as *mut AtaPassThroughDirect as *mut c_void, size_of::<AtaPassThroughDirect>() as u32, 0, &mut evt as *mut KEVENT, &mut iosb as *mut IO_STATUS_BLOCK)
	};
	...
	if irp!=core::ptr::null_mut::<IRP>()
	{
		...
		if st==STATUS_PENDING
		{
			unsafe
			{
				let _=KeWaitForSingleObject(&mut evt as *mut KEVENT as *mut c_void, _KWAIT_REASON::Executive, _MODE::KernelMode as i8, 0, core::ptr::null_mut());
...
fn get_device_list(driver:*mut DRIVER_OBJECT,device_count:&mut u32)->Result<*mut PDEVICE_OBJECT,NTSTATUS>
{
	...
	{
		IoEnumerateDeviceObjectList(driver, core::ptr::null_mut(), 0, device_count)
	};
...
pub fn find_disk(disk_obj:&mut DiskObject)->NTSTATUS
{
	...
	let mut st:NTSTATUS=unsafe
	{
		...
		ObReferenceObjectByName(&mut disk_drv_name,OBJ_CASE_INSENSITIVE,core::ptr::null_mut(),0,*IoDriverObjectType,_MODE::KernelMode as i8,core::ptr::null_mut(),&mut disk_drv_obj as *mut PDRIVER_OBJECT as *mut PVOID)
	};
	if NT_SUCCESS(st)
	{
		...
		match r
		{
			Ok(device_list)=>
			{
				...
				{
					let dev_obj:PDEVICE_OBJECT=unsafe
					{
						*device_list.offset(i as isize)
					};
